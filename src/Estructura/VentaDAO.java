...
public class VentaDAO {
    ...
    public int IdVenta(){
        int id = 0;
        String sql = "SELECT MAX(id) FROM tVentas";
        try {
    ...
    public int RegistrarVenta(Venta v){
        String sql = "INSERT INTO tVentas (cliente, vendedor, total, fecha) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public int RegistrarDetalle(Detalle Dv){
       String sql = "INSERT INTO tDetalle (id_pro, cantidad, precio, id_venta) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public boolean ActualizarStock(int cant, int id){
        String sql = "UPDATE tProducto SET stock = ? WHERE id = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1,cant);
    ...
    public List Listarventas(){
       ...
       String sql = "SELECT c.id AS id_cli, c.nombre, v.* FROM tCliente c INNER JOIN tVentas v ON c.id = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
    ...
    public Venta BuscarVenta(int id){
        Venta cl = new Venta();
        String sql = "SELECT * FROM tVentas WHERE id = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            Encabezado.addCell("");
            //info empresa
            String config = "SELECT * FROM tEmpresa";
            String mensaje = "";
            try {
                con = cn.getConnection();
            ...
            String prove = "SELECT * FROM tCliente WHERE id = ?";
            try {
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.id_pro,d.id_venta, d.precio, d.cantidad, p.id, p.nombre FROM tDetalle d INNER JOIN tProducto p ON d.id_pro = p.id WHERE d.id_venta = ?";
            try {
